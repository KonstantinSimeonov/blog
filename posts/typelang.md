...
A cute trick to understand it is to think about it like that:
...
By itself, conditional typing doesn't look that big of deal, but when we combine it with generic types, it becomes pretty powerful:
...
- `never` is to types what `null` is to values (somewhat wrong, but helps develop an intuition for it's use)
...
In reality, we also perform an `extends` check, so the javascript code would look more like that:
...
We went a bit overboard with the haskell example - we used pattern matching in a function. Let's see if we can do the same with a typescript type - a generic type (might as well call it a type function at this point) that extracts the `x` property of an object:
...
Disregarding the obviously horrible performance, the above is a correct solution. Since we're armed with recursive types, generics and conditionals, we can try something like the following:
...
- pattern matching or conditional expressions + destructuring if you will (`a extends infer b ? b : ...`, allows us to do conditional computing)
- functions (generics)
- recursion (allows us to repeat some calculation, not unlike loops do)
